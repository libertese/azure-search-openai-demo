...
def dbInsert(question, answer):
    ...
    sql = "insert into questions (feedback) values ('" +inserted_id + "') where  questionid = " +inserted_id 
    curs2.execute(sql)
    connection.commit()
    return inserted_id
...
SUFFIX = """\nQuestion: {input}
{agent_scratchpad}"""
PREFIX = "Answer questions in Portuguese from Portugal  as shown in the following examples, by splitting the question into individual search or lookup actions to find facts until you can answer the question. " \
