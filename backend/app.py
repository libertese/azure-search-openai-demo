...
from feedback.feedback import userfeedback
...
def placefeedback(questionId,feedback):
    ...
    if feedback :
             ...
             _feedback = "False"
    sql = "update public.questions set feedback='"+_feedback+"' where  questionid =  cast('"+questionId+"' AS INTEGER)"
    curs2.execute(sql)
    connection.commit()
...
#minha_thread = threading.Thread(target=chat)
...
def feedback():
    questionId =request.json["feedbackId"]
    feedback = request.json["feedback"]
    try:
         placefeedback(questionId,feedback)
         return 
    except Exception as e:
...
#@app.route("/chat", methods=["POST"])
#def chat():
#    thread = threading.Thread(target=chat02)
#    thread.start()
